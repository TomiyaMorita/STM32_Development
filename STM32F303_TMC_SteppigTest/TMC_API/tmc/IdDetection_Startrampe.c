...
// Helper macros
#define ID_CLK_LOW()   GPIOB->BSRRH=BIT9    // set id clk signal to low
#define ID_CLK_HIGH()  GPIOB->BSRRL=BIT9    // set id clk signal to high
#define ID_CLK_STATE   (GPIOB->ODR & BIT9)  // get id clk signal level
#define ID_CH0_STATE   (GPIOB->IDR & BIT0)  // get id signal level for this channel
#define ID_CH1_STATE   (GPIOB->IDR & BIT1)  // get id signal level for this channel
...
void IDDetection_init(void)
{
	...
	TIM1->PSC = 5;       // prescaler -> 0.1us
	TIM1->ARR = 100000;  // timeout   -> 10ms
...
static int detectID_Monoflop(IdAssignmentTypeDef *ids)
{
	...
	{
	case MONOFLOP_INIT:
		TIM_Cmd(TIM1, DISABLE);  // stop timer
	...
	case MONOFLOP_DONE:
		...
		if(IdState.ch1.state == ID_STATE_DONE)
		{
			...
			ids->ch1.id = assign(TIM1->CCR2 - TIM1->CCR1);
		...
		{
			// Assign the ID derived from the ID pulse duration
			ids->ch2.id = assign(TIM1->CCR4 - TIM1->CCR3);
