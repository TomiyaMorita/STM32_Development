...
void RMDZero(){
	...
	int ArmMotor_deg;
	ArmMotor_deg=(int)(((-p_getdata_s1-p_getdata_s2+M_PI)*180)/M_PI);
	get_RMD_deg=first_deg_pos+100*ArmMotor_deg;
...
//	int i = 0;
	...
	TxHeader.TransmitGlobalTime = DISABLE;
	RMDTx_can_data[1] = 0x00;
	RMDTx_can_data[2] = 0x00;
	RMDTx_can_data[3] = 0x00;
	RMDTx_can_data[4] = 0x00;
	RMDTx_can_data[5] = 0x00;
	RMDTx_can_data[6] = 0x00;
	RMDTx_can_data[7] = 0x00;
	switch(Writecom){
		case 0x31:
			RMDTx_can_data[0] = 0x31;	//PI設定
			RMDTx_can_data[2] = UART1_Data[2];	//anglePidKp
			RMDTx_can_data[3] = UART1_Data[3];	//anglePidKi
			RMDTx_can_data[4] = UART1_Data[4];	//speedPidKp
			RMDTx_can_data[5] = UART1_Data[5];	//speedPidKi
			RMDTx_can_data[6] = UART1_Data[6];	//iqPidKp
			RMDTx_can_data[7] = UART1_Data[7];	//iqPidKi
		case 0x32:
		...
		case 0x34:
			...
			RMDTx_can_data[4] = UART1_Data[4];	//speedI
			RMDTx_can_data[5] = UART1_Data[5];	//TorqueP
			RMDTx_can_data[6] = UART1_Data[6];	//TorqueI
			RMDTx_can_data[7] = UART1_Data[7];	//PosI
			break;
		case 0xA1:
			RMDTx_can_data[0] = 0xA1;	//Torque設定
			RMDTx_can_data[4] = UART1_Data[4];	//currentlow
			RMDTx_can_data[5] = UART1_Data[5];	//currenthigh
			break;
		case 0xA4:
			RMDTx_can_data[0] = 0xA4;
			RMDTx_can_data[1] = 0x00;
			...
			RMDTx_can_data[4] = UART1_Data[4];	//poslow
			RMDTx_can_data[5] = UART1_Data[5];
			RMDTx_can_data[6] = 0x00;
			RMDTx_can_data[7] = 0x00;	//poshigh
			break;
		default:
			end_flag=1;
			break;
	...
	switch(Readcom){
		case 0x30:
			RMDTx_can_data[0] = 0x30;
			break;
		case 0x33:
			RMDTx_can_data[0] = 0x33;
			break;
		case 0x9C:
			RMDTx_can_data[0] = 0x9C;
			break;
		case 0xA1:
			RMDTx_can_data[0] = 0xA1;
		case 0x90:
			RMDTx_can_data[0] = 0x90;
			break;
		default:
			end_flag=1;
			break;
...
void SetUpRMD(){
	RMDTx_can_data[4] = 0x50;
	RMDTx_can_data[5] = 0x46;
	Writecom = 0xA4;
	RMDCommand();
}
void RMDLeftTurn(){
	RMDTx_can_data[4] = 0xE0;
	RMDTx_can_data[5] = 0x2E;
	Writecom=0xA4;
	RMDCommand();
}
void RMDRightTurn(){
	RMDTx_can_data[4] = 0xC0;
	RMDTx_can_data[5] = 0x5D;
	Writecom=0xA4;
	RMDCommand();
}
void checkRMDdata(){
...
void RMDPIDSetting(){
	RMDTx_can_data[2] = 0x64;
	RMDTx_can_data[3] = 0x64;
	RMDTx_can_data[4] = 0x28;
	RMDTx_can_data[5] = 0xE;
	RMDTx_can_data[6] = 0x14;
	RMDTx_can_data[7] = 0x14;
	Writecom=0x31;
	RMDCommand();
}
void UART1_InIt(){
...
uint8_t mode_select;
int16_t signed_transmit_receive_data[20];
uint16_t unsigned_transmit_receive_data[20];
